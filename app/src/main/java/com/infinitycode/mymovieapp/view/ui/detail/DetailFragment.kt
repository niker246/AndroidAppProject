...
class DetailFragment : Fragment(R.layout.fragment_detail), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        movieResult = requireArguments().getParcelable(Constants.MOVIE)
    }
    ...
    private fun updateUI(resource: Resource<MovieDetail>?) {
        if (resource?.data != null) {
            if (resource.data.genres != null) {
                val stringCommaGenre = resource.data.genres.joinToString { it.name }
                tvGnreValue.text = stringCommaGenre
            }
            ...
            snackbar =
                Snackbar.make(nestedDetail, "Has lost connection", Snackbar.LENGTH_INDEFINITE)
